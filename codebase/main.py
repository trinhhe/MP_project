...
from regressor.util import proj_3D_to_2D
...
#     print(lol.items())
        x = torch.rand(4, 5, 3).float()
        a = torch.rand(4).float()
        b = torch.rand(4).float()
        c = torch.rand(4).float()
        d = torch.rand(4).float()
        lol = proj_3D_to_2D(x, a,b,c,d)
        print(lol.shape)
        print(lol)
