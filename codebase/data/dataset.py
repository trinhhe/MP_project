...
class H36MDataset(torch.utils.data.Dataset):
        ...
        for subject in self.subjects:
            ...
            for action in actions:
                for cam_idx, cam in enumerate(cam_params):
                    img_dir = join(self.img_folder, subject,
                                   'Images', action + '.' + cam['id'])
    ...
    def __getitem__(self, idx):
        ...
        if crop_ver == 0:
            image_crop = crop(
                image, points_dict['center_img'], points_dict['scale_img'], self.img_size).astype(np.float32)
            image_crop = (image_crop / 255.0 - self.mean) / \
                self.std  # normalize
        ...
        if crop_ver == 1:
            ...
            image_crop, t_no_scale, h_rot = crop_new(
                image, points_dict['center_img'], sc * points_dict['scale_img'], self.img_size, rot=rot, flip=flip)
            ...
            points_dict['root_orient'], points_dict['pose_body'], points_dict['pose_hand'] = pose_processing(
                full_pose, r=rot, f=flip)
        ...
        if crop_ver == 1:
            ...
            image_crop = image_crop + noise_factor * \
                gaussian_noise.astype('float32')
            # # image_crop = np.clip(image_crop, a_min=0., a_max=1.)  # clip
            # # plt.imshow(image_crop);plt.show()
            #
            # 2.) Add random flip
        ...
        if self.mode in ['val']:
            if image.shape[0] == 1000:
                ...
                image = np.pad(image, pad_width=(
                    (0, 2), (0, 0), (0, 0)), mode='reflect')

            # -> PyTorch tensor format
            data_out.update({'image': image.transpose([2, 0, 1])})
    ...
    def collate_fn(data):
        ...
        for sample in data:
            batch.append(
                {key: val for key, val in sample.items() if key != 'file_id'})
