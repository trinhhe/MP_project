...
'''
    https://github.com/MandyMo/pytorch_HMR/blob/master/src/LinearModel.py
'''
    ...
    def _check(self):
        while True:
            if not isinstance(self.fc_layers, list):
                print('fc_layers require list, get {}'.format(
                    type(self.fc_layers)))
                break

            if not isinstance(self.use_dropout, list):
                print('use_dropout require list, get {}'.format(
                    type(self.use_dropout)))
                break

            if not isinstance(self.drop_prob, list):
                print('drop_prob require list, get {}'.format(
                    type(self.drop_prob)))
                break

            if not isinstance(self.use_ac_func, list):
                print('use_ac_func require list, get {}'.format(
    ...
    def create_layers(self):
        ...
        for _ in range(l_fc_layer - 1):
            self.fc_blocks.add_module(
                ...
                module=nn.Linear(
                    in_features=self.fc_layers[_], out_features=self.fc_layers[_ + 1])
            )
...
if __name__ == '__main__':
    ...
    net = LinearModel(fc_layers, use_dropout,
                      drop_prob, use_ac_func).to(device)
    print(net)
    nx = np.zeros([2, 2048])
